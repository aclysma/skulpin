...
impl MtlSkiaContext {
  pub fn new(
    ...
    let mtl_queue_handle = queue.metal_queue().unwrap();//.queue().queue().lock().unwrap();
...
impl MtlSkiaSurface {
    pub fn get_image_from_skia_texture(texture: &skia_safe::gpu::BackendTexture) -> metal_rs::Texture {
