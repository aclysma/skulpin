...
impl VkDevice {
    ...
    fn choose_physical_device(
        ...
        for physical_device in physical_devices {
            if let Some((score, queue_family_indices)) =
                Self::get_score_and_queue_families_for_physical_device(
                    instance,
                    &physical_device,
                    surface_loader,
                    surface,
    ...
    fn get_score_and_queue_families_for_physical_device(
        ...
        if features.sampler_anisotropy == vk::FALSE {
            info!(
                "Found unsuitable device '{}', does not support sampler_anisotropy",
                device_name
            );
            return Ok(None);
        }
        ...
        if let Some(queue_family_indices) = queue_family_indices {
            ...
            info!(
                "Found unsuitable device '{}', could not find queue families",
                device_name
            );
            trace!("{:#?}", properties);
            Ok(None)
    ...
    fn find_queue_families(
        ...
        for (queue_family_index, queue_family) in queue_families.iter().enumerate() {
            ...
            if queue_family.queue_flags & ash::vk::QueueFlags::GRAPHICS
